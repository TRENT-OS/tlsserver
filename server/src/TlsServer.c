...
// The TlsServer_CLIENT_ASSIGN_BADGES() macro will start assigning
...
TlsServer_Client_t instanceClients[MAX_CLIENTS_NUM];
    ...
    int clientId)
{
    ...
    {
        if ((instanceClients[i].inUse) && (instanceClients[i].clientId == clientId))
        {
...
static TlsServer_Client_t*
getClient_by_sockethandle(
    int socket_handle)
{
    for (int i = 0; i < MAX_CLIENTS_NUM; i++)
    {
        if ((instanceClients[i].inUse) && (instanceClients[i].hSocket.handleID == socket_handle))
        {
            return &instanceClients[i]; 
        }
    }
...
OS_Error_t closeClientConnection(
    TlsServer_Client_t* client)
{
    ...
    {
        Debug_LOG_ERROR("Socket of client ID %u is not connected", client->clientId);
    ...
    void)
{
    ...
    for (int i = 0; i < numberConnectedClients; i++)
    {
        ...
        if ((tlsServer_rpc_enumerate_badge(i) < MIN_BADGE_ID) ||
            (tlsServer_rpc_enumerate_badge(i) >= MIN_BADGE_ID + numberConnectedClients))
        {
            Debug_LOG_ERROR(
                "[TlsServer '%s'] Badge Id is out of bounds: %d, Min: %d, Max: %d",
                get_instance_name(),
                tlsServer_rpc_enumerate_badge(i),
                MIN_BADGE_ID,
                MIN_BADGE_ID + numberConnectedClients - 1);
        ...
        if ((err = OS_Crypto_init(&instanceClients[i].hCrypto, &cryptoCfg)) != OS_SUCCESS)
        {
    ...
    const uint16_t port)
{
    ...
    if (client->state == CONNECTIONERROR)
    {
        // confirm once to the client, then go to connected state
        client->state = DISCONNECTED;
        return client->error;
    ...
    client->state = CONNECTIONWAITING;
    ...
    void)
{
    ...
    Debug_LOG_DEBUG("tlsServer_rpc_disconnect()");
    ...
    size_t* dataSize)
{
    ...
    {
        Debug_LOG_ERROR("TLS connection of client ID %u is not established", client->clientId);
        return OS_ERROR_INVALID_STATE;
    }
    ...
    void)
{
    ...
    if ((client->state == PENDINGHANDSHAKE) ||
        (client->state == HANDSHAKED))
    {
        client->state = CONNECTED;
    }

    Debug_LOG_ERROR("tlsServer_rpc_reset()");
    ...
    void)
{
    ...
    for (;;)
    {
        ...
        Debug_LOG_TRACE("[TlsServer '%s'] Event Notification received", get_instance_name());
        ...
        if (err != OS_SUCCESS)
        {
            Debug_LOG_ERROR("OS_NetworkSocket_getPendingEvents() failed, code %d", err);
            continue;
        }

        Debug_LOG_TRACE("OS_NetworkSocket_getPendingEvents: received %d events ", numberOfSocketsWithEvents);
        ...
        for (int i = 0; i < numberOfSocketsWithEvents; i++)
        {
            ...
            {
                Debug_LOG_ERROR("handle event - received invalid handle: %d", event.socketHandle);
                continue;
            }

            TlsServer_Client_t* client;
            client = getClient_by_sockethandle(event.socketHandle);

            if (client == NULL)
            {
                Debug_LOG_ERROR("OS_NetworkSocket_getPendingEvents: unknown handle received: %d",  event.socketHandle);
                continue;
            }
            ...
            {
                Debug_LOG_ERROR("OS_NetworkSocket_getPendingEvents: OS_SOCK_EV_FIN for clientId %d, handle: %d", client->clientId,  event.socketHandle);
            ...
            {
                client->state = CONNECTIONCONFIRMED;
                Debug_LOG_DEBUG("Connection established for clientId %d, handle: %d", client->clientId, event.socketHandle);
            }

            // new client connection (TCP only)
            if (event.eventMask & OS_SOCK_EV_CONN_ACPT)
            {
                Debug_LOG_ERROR("OS_NetworkSocket_getPendingEvents: Unexpected event - OS_SOCK_EV_CONN_ACPT handle: %d", event.socketHandle);
            }
            ...
            {
                // do nothing
                // closing of the connection will be handled by TlsServer_disconnect()
            }

            // Error received - print error
            if (event.eventMask & OS_SOCK_EV_ERROR)
            {
                Debug_LOG_ERROR("OS_NetworkSocket_getPendingEvents: OS_SOCK_EV_ERROR handle: %d, code: %d", event.socketHandle, event.currentError);
                // handle failed connection events:
                if (client->state == CONNECTIONWAITING)
                {
                    client->state = CONNECTIONERROR;
